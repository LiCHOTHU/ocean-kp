...
class SetEncoder(FlattenMlp):
    def __init__(self, *args, **kwargs):
        self.save_init_params(locals())
        super().__init__(*args, **kwargs)
        # assert self.set_output_size != None
        self.fcfc = nn.Linear(self.output_size, 100)
        self.hidden_init(self.fcfc.weight)
        self.fcfc.bias.data.fill_(self.b_init_value)
        self.__setattr__("set_fc1", self.fcfc)

        self.fcfc2 = nn.Linear(100, self.set_output_size)
        self.fcfc2.weight.data.uniform_(-self.init_w, self.init_w)
        self.fcfc2.bias.data.uniform_(-self.init_w, self.init_w)


    def forward(self, *inputs, **kwargs):
        flat_outputs = self.hidden_activation(super().forward(*inputs, **kwargs))
        set_outputs = self.set_activation(flat_outputs, dim=1)
        if self.set_activation == torch.max:
            set_outputs = set_outputs[0]
        h = self.hidden_activation(self.fcfc(set_outputs))
        preactivation = self.fcfc2(h)
        output = self.output_activation(preactivation)
        print (output.size())
        if kwargs.get('return_preactivations', False):
            return output, preactivation
        else:
            return output
