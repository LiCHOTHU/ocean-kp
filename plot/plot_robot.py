...
def plotArg(game, ax):
    ...
    expert_reward = -6.22
    random_reward = -13.46
    for dim, cat, cdim, lam, tem, ann, unit, dc, cc in itertools.product(dims, cats, cdims, lams, tems, anns, units, dcs, ccs):
        criterion = 'dim-%s-cat-%s-cdim-%s-lam-%s-tem-%s-ann-%s-unit-%s-dc-%s-cc-%s/'\
                        %(dim, cat, cdim, lam, tem, ann, unit, dc, cc)
        print (criterion)
        datas = get_data(prefix='/test/oyster/cat-new/two-categorical-point-robot/', \
...
if __name__ == '__main__':
    ...
    fig.tight_layout()
    fig.savefig('test-two-cat.pdf', bbox_extra_artists=(lgd,), bbox_inches='tight')
