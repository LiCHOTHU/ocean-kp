    ...
    def _take_step(self, indices, context):
        ...
        if os.environ['DEBUG'] != '0':
            ...
            if self.use_information_bottleneck:
                ...
                if self.kl_anneal:
                    kl_loss = self.kl_lambda * (torch.abs(kl_div_disc - disc_cap_cur) + torch.abs(kl_div_cont - cont_cap_cur) + torch.abs(kl_div_dir - dir_cap_cur)) * (1 - math.exp(-(self._n_train_steps_total+100) / 20000.0))
                else:
                    kl_loss = self.kl_lambda * (torch.abs(kl_div_disc - disc_cap_cur) + torch.abs(kl_div_cont - cont_cap_cur) + torch.abs(kl_div_dir - dir_cap_cur))
                if False:
        ...
        if self.use_information_bottleneck:
            ...
            if self.kl_anneal == 'mono':
                kl_loss = self.kl_lambda * (torch.abs(kl_div_disc - disc_cap_cur) \
                    ...
                    + torch.abs(kl_div_seq_dir - dir_cap_cur)) * (1 - math.exp(-(self._n_train_steps_total+100) / 100000.0))
