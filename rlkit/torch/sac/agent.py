...
class PEARLAgent(nn.Module):
    ...
    def __init__(self,
        ...
        if self.glob:
            self.register_buffer('z', torch.zeros(1, self.cont_latent_dim + self.latent_dim * self.num_cat + self.dir_latent_dim * self.num_dir))
    ...
    def clear_z(self, num_tasks=1, batch_size=1, traj_batch_size=1): #! check when it happens, because it looks fishy for batch size part
        '''
        ...
        if True:
            ...
            if self.unitkl:
                ...
                if self.dir_latent_dim > 0 and self.constraint == 'logitnormal' and False: # actually gaussians should not have kl divergence in each step!!
                    prior = torch.distributions.Normal(ptu.zeros(self.dir_latent_dim), ptu.ones(self.dir_latent_dim)*np.sqrt(self.var))
                ...
                if self.glob:
                    ...
                    if self.dir_latent_dim > 0:
                        if self.constraint == 'deepsets':
                            ...
                            kl_div_dir = torch.sum(torch.distributions.kl.kl_divergence(posteriors, prior))
                    if self.cont_latent_dim > 0:
                        kl_div_cont = torch.sum(0.5*(-torch.log(self.z_c_vars)+self.z_c_vars+self.z_c_means*self.z_c_means-1)) #! still needs verify
    ...
    def infer_posterior(self, context, ff=False): 
        ...
        if self.dir_latent_dim > 0 and self.constraint == 'deepsets':
            ...
            params = self.context_encoder(context)
            params = params.view(context.size(0), -1, self.num_cat*self.latent_dim + 2*self.cont_latent_dim + self.num_dir*self.dir_latent_dim*2)
        ...
        if self.cont_latent_dim > 0:
            params_cont = params[..., self.num_cat*self.latent_dim:self.num_cat*self.latent_dim+2*self.cont_latent_dim]
            ...
            if False:
                z_params_c = [_product_of_gaussians(m, s) for m, s in zip(torch.unbind(mu_c), torch.unbind(sigma_squared_c))]
    ...
    def sample_z(self):
        if True: #!! I think this one might be quicker, test it before push
            ...
            if self.latent_dim > 0:
                ...
                z = F.softmax(logit, dim=1).view(-1, self.num_cat, self.latent_dim).view(-1, self.num_cat * self.latent_dim)
            if self.cont_latent_dim > 0:
            ...
            soft_cats = [torch.distributions.RelaxedOneHotCategorical(ptu.FloatTensor([self.temperature]), m) for m in torch.unbind(self.z_means)]
    ...
    def forward(self, obs, context, trajectories, indices_in_trajs, do_inference, compute_for_next, is_next):
        ...
        policy_outputs = self.policy(in_, reparameterize=True, return_log_prob=True) #!! add flag, we donot need reparameterize for discrete action space, check
    ...
    def clear_sequence_z(self, num_tasks=1, batch_size=1, traj_batch_size=1): #! check when it happens, because it looks fishy for batch size part
        '''
        reset q(z|c) to the prior
        sample a new z from the prior
        '''
