...
def get_data(prefix, criterion, er, rr, sstr):
    ...
    with cd(prefix):
        ...
        for path in paths:
            ...
            for f in files:
                root_f = f[:-12]
                if len(glob.glob(os.path.join(root_f, "online-train-epoch600.pkl"))) == 0:
                    continue
...
if __name__ == '__main__':
    ...
    print ('save pdf')
    if len(idxs) > 1:
        fig.savefig('figs/test-update-ml1-%s.pdf'%sstr, bbox_extra_artists=(lgd,), bbox_inches='tight')
    else:
        fig.savefig('figs/test-update-%s.pdf'%task_name[idxs[0]], bbox_extra_artists=(lgd,), bbox_inches='tight')
