...
class Loss(_Loss):
    ...
    def forward(self, Kp_fr, Kp_to, anc_fr, anc_to, att_fr, att_to, r_fr, t_fr, r_to, t_to, mesh, faces, scale, cate, geodesic, curvature, quadric):
        sym_or_not = False
    ...
    #
        ...
        elif self.loss_surf_type == 'quadric':
            full_mesh = pymesh.form_mesh(mesh.squeeze().cpu().numpy(), faces.squeeze().cpu().numpy())
            num_kp = gt_Kp_fr.shape[1]

            sq_dist, face_indices_fr, closest_points_fr = pymesh.distance_to_mesh(full_mesh, gt_Kp_fr.squeeze().detach().cpu().numpy())
            faces_select_fr = faces[:, face_indices_fr, :]
            vert_indices_fr = faces_select_fr.view(3*num_kp)
            quadric_select_fr = (quadric[:, vert_indices_fr, :]).to(gt_Kp_fr.device)
            gt_Kp_fr_repeat = torch.repeat_interleave(gt_Kp_fr, 3, dim=1).to(gt_Kp_fr.device)
            quadric_loss_fr = self.Quadric(quadric_select_fr, gt_Kp_fr_repeat)

            sq_dist, face_indices_to, closest_points_to = pymesh.distance_to_mesh(full_mesh, gt_Kp_to.squeeze().detach().cpu().numpy())
            faces_select_to = faces[:, face_indices_to, :]
            vert_indices_to = faces_select_to.view(3 * num_kp)
            quadric_select_to = (quadric[:, vert_indices_to, :]).to(gt_Kp_to.device)
            gt_Kp_to_repeat = torch.repeat_interleave(gt_Kp_to, 3, dim=1).to(gt_Kp_to.device)
            quadric_loss_to = self.Quadric(quadric_select_to, gt_Kp_to_repeat)

            loss_surf = (quadric_loss_fr + quadric_loss_to).contiguous() / 2.0
        ...
        '''
        print("att: ",loss_att_scaled)
        print( "dis: ", Kp_dis_scaled)
        print("cent: ",Kp_cent_dis_scaled)
        print("rot: ",loss_rot_scaled)
        print("surf: ",loss_surf_scaled)
        print("sep: ",loss_sep_scaled)
        '''
    ...
    def Quadric(self, Q, V, average=False):
