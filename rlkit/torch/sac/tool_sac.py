...
class PEARLSoftActorCritic(MetaRLAlgorithm):
    def __init__(
        ...
        self.max_disc_cap = max_disc_cap
        self.max_cont_cap = max_cont_cap
        self.max_dir_cap = max_dir_cap
        ...
        if self.kl_anneal == 'cycle':
            self.annealer = frange_cycle_linear(self.num_iterations * self.num_train_steps_per_itr, n_cycle=20000,
                                                ratio=0.7)
        # import pdb
        # pdb.set_trace()
    ...
    def sample_data(self, indices, encoder=False):
        ...
        for idx in indices:
            if encoder:
                ...
                # pdb.set_trace()
                batch = ptu.np_to_pytorch_batch(
                    self.replay_buffer.random_batch(idx, batch_size=self.batch_size, sequence=self.recurrent,
                                                    traj_batch_size=self.traj_batch_size))
    ...
    def _take_step(self, indices, context):
        ...
        if os.environ['DEBUG'] != '0':
            ...
            if self.use_information_bottleneck:
                kl_div_disc, kl_div_cont, kl_div_dir = self.agent.compute_kl_div()
                disc_cap_cur = min(self.max_disc_cap * self._n_train_steps_total / 20000., self.max_disc_cap)
                cont_cap_cur = min(self.max_cont_cap * self._n_train_steps_total / 20000., self.max_cont_cap)
                dir_cap_cur = min(self.max_dir_cap * self._n_train_steps_total / 20000., self.max_dir_cap)
                if self.kl_anneal:
                    kl_loss = self.kl_lambda * (torch.abs(kl_div_disc - disc_cap_cur) + torch.abs(
                        kl_div_cont - cont_cap_cur) + torch.abs(kl_div_dir - dir_cap_cur)) * (
                                          1 - math.exp(-(self._n_train_steps_total + 100) / 20000.0))
                else:
                    kl_loss = self.kl_lambda * (torch.abs(kl_div_disc - disc_cap_cur) + torch.abs(
                        kl_div_cont - cont_cap_cur) + torch.abs(kl_div_dir - dir_cap_cur))
                if False:
        ...
        if self.use_information_bottleneck:
            ...
            if self.kl_anneal == 'mono':
                kl_loss = self.kl_lambda * (torch.abs(kl_div_disc - disc_cap_cur) \
                                            + torch.abs(kl_div_cont - cont_cap_cur) \
                                            + torch.abs(kl_div_dir - dir_cap_cur) \
                                            + torch.abs(kl_div_seq_disc - disc_cap_cur) \
                                            + torch.abs(kl_div_seq_cont - cont_cap_cur) \
                                            + torch.abs(kl_div_seq_dir - dir_cap_cur)) * (
                                      1 - math.exp(-(self._n_train_steps_total + 100) / 100000.0))
                ...
                kl_loss = self.kl_lambda * (torch.abs(kl_div_disc - disc_cap_cur) \
                                            ...
                                            + torch.abs(kl_div_seq_disc - disc_cap_cur) \
                                            + torch.abs(kl_div_seq_cont - cont_cap_cur) \
                                            + torch.abs(kl_div_seq_dir - dir_cap_cur)) * float(next(self.annealer))
    ...
    def get_epoch_snapshot(self, epoch):
        ...
        if self.recurrent:
            ...
            # snapshot['recurrent_context_optimizer'] = self.recurrent_context_optimizer.state_dict()
        if self.glob:
            snapshot['context_encoder'] = self.agent.context_encoder.state_dict()
            # snapshot['context_optimizer'] = self.context_optimizer.state_dict()
        return snapshot
