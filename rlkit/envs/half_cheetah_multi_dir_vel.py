...
class HalfCheetahMultiDirVelEnv(HalfCheetahEnv):
    ...
    """
    def __init__(self, task={}, n_tasks=2, randomize_tasks=True, n_vels=3, min_vel=-5, max_vel=5, max_eps=700, seed=0):
        self._task = task
            ...
            while num_sampled < num_tasks:
                ...
                if num_vels == 3:
                    if (sampled_data[0] < 0 and sampled_data[1] < 0 and sampled_data[2] < 0) or (sampled_data[0] > 0 and sampled_data[1] > 0 and sampled_data[2] > 0):
                        continue
                    # if np.std(sampled_data) < 1.8:
                    #     continue
                velocities.append(sampled_data)
                num_sampled += 1
            velocities = np.array(velocities)
        else:
            velocities = np.random.uniform(self._min_vel, self._max_vel, size=(num_tasks, num_vels))
