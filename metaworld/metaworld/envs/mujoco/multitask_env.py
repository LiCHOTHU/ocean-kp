...
class Iter():
    def __init__(self, leng):
        self.leng = leng
        self.step = 0
        self.lis = np.random.permutation(self.leng)

    def next(self, num):
        assert num <= self.leng
        if self.step + num > self.leng:
            left_num = self.lis[self.step:]
            self.lis = np.random.permutation(self.leng)
            right_num = self.lis[:self.step+num-self.leng]
            all_num = np.concatenate([left_num, right_num])
            self.step = self.step + num - self.leng
        elif self.step + num == self.leng:
            all_num = self.lis[self.step:]
            self.lis = np.random.permutation(self.leng)
            self.step = 0
        else:
            all_num = self.lis[self.step:self.step + num]
...
class MultiClassMultiTaskEnv(MultiTaskEnv):
    ...
    def __init__(self,
        ...
        self._active_task = 0
        self.sim = self.active_env.sim # for consistent use of simulation
        self._check_env_list()

    def seed(self, seed=0):
        for task in self._task_envs:
            # print (task)
            task.goal_space.np_random.seed(seed)

    def reorder_env(self, task_names):
        self.new_task_envs = []
        for task_name in task_names:
            self.new_task_envs.append(self._task_envs[self._task_names.index(task_name)])
        self._task_names = task_names
        self._task_envs = self.new_task_envs
        self._env_discrete_index = {
            task: i
            for i, task in enumerate(self._task_names)
        }

    def discretize_goal_space(self, discrete_goals, seed=0):
        for task, goals in discrete_goals.items():
            if task in self._task_names:
                ...
                self._task_envs[idx].discretize_goal_space(
                    self._discrete_goals[task], seed
                )
    ...
    def setup_iter(self):
        self.train_iter = Iter(self.n_train_goals)
    ...
    @property
    def _goal(self):
        return self.active_env._state_goal
    
    def setup_ML1_test_goals(self):
        raise Exception('deprecated')
        tasks = [0 for _ in range(self.n_test_goals)]
        goals = range(self.n_train_goals, self.n_train_goals+self.n_test_goals)
        self.test_tasks = [
            dict(task=t, goal=g)
            for t, g in zip(tasks, goals)
        ]

    def setup_ML10_test_goals(self):
        raise Exception('deprecated')
        tasks = []
        for i in range(10, 15):
            tasks.extend([i]*(self.n_test_goals//5))
        goals = range(self.n_train_goals, self.n_train_goals+self.n_test_goals)
        self.test_tasks = [
            dict(task=t, goal=g)
            for t, g in zip(tasks, goals)
    ...
    def sample_train_tasks(self, meta_batch_size):
        assert self.ML1
        # assert len(self._task_envs) == 1
        assert self._sample_goals
        tasks = [0 for _ in range(meta_batch_size)]
        goals = self.train_iter.next(meta_batch_size)
        tasks_with_goal = [
            dict(task=t, goal=g)
            for t, g in zip(tasks, goals)
        ]
