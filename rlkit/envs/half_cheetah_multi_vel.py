...
@register_env('cheetah-multi-vel')
class HalfCheetahMultiVelEnv(HalfCheetahEnv):
    ...
    """
    def __init__(self, task={}, n_tasks=2, randomize_tasks=True, n_vels=3, max_vel=5, max_eps=700, seed=0):
        self._task = task
        ...
        super(HalfCheetahMultiVelEnv, self).__init__()
        self.seed(seed)
    ...
    def sample_tasks(self, num_tasks, num_vels):
        np.random.seed(1337)
        velocities = np.random.uniform(0.0, self._max_vel, size=(num_tasks, num_vels))
        # change_steps = np.sort(np.random.uniform(0, self._max_eps, size=(num_tasks, num_vels - 1)))
